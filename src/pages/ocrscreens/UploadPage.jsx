...
const UploadPage = () => {
    ...
    fileInput.onchange = async (e) => {
      ...
      for (const [index, file] of files.entries()) {
        ...
        if (
          ...
          !file.name.toLowerCase().match(/\.(pdf|png|svg|webp|jpg|jpeg)$/)
        ) {
          alert(
            `File ${file.name} is not a supported type. Please upload PDF, PNG, SVG, WEBP, JPEG, or JPG files only.`
          );
          continue;
        }
  ...
  const handleMouseMove = (e) => {
    ...
    setCropArea((prev) => {
      ...
      if (cropResizeMode === "move") {
        newArea.x = Math.max(
          0,
          Math.min(rect.width - newArea.width, prev.x + deltaX)
        );
        newArea.y = Math.max(
          0,
          Math.min(rect.height - newArea.height, prev.y + deltaY)
        );
              ...
              Array.from(
                ...
                (_, index) => (
                  <div key={index} className="mb-4 flex justify-center">
                    <Page pageNumber={currentPdfPage} width={500} />
                  </div>
                )
              )}
          </Document>
  ...
  const processFilesWithFastAPI = async (filesToProcess) => {
    ...
    try {
      ...
      if (!fastapiUrl) {
        throw new Error(
          "FastAPI server not found. Make sure it's running on port 8010."
        );
      }
                ...
                <div className="flex flex-col gap-4">
                  {uploadedFiles.length === 0 ? (
                    ...
                    uploadedFiles.map((file) => {
                      const progress =
                        uploadProgress[file.id] || file.progress || 0;
                      const status =
                        ...
                        <div
                          ...
                          onClick={() =>
                            setCurrentPreviewIndex(
                              uploadedFiles.findIndex((f) => f.id === file.id)
                            )
                          }
                        >
                          <file.Icon
              ...
              <div className="w-full flex items-center justify-center mb-4 gap-5">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    disabled={
                      currentPreviewIndex === 0 || uploadedFiles.length === 0
                    }
                  >
            ...
            <div className="gap-y-3 flex flex-wrap md:flex-nowrap items-center justify-center w-full px-4 py-3 bg-[#E7EDF2] rounded-[15px] md:gap-3">
              ...
              <div className="flex items-center gap-1 xm:gap-2 px-4 border-r border-[#21527D]/20">
                <button
                  ...
                  disabled={
                    currentPreviewFile?.file.type !== "application/pdf" ||
                    currentPdfPage === 1
                  }
                >
                ...
                <div className="font-avenir font-normal text-[16px] leading-[100%] text-[#000000]">
                  {currentPreviewFile?.file.type === "application/pdf"
                    ? pdfPages[currentPreviewFile.id] || 1
                    : 1}
                </div>
                ...
                <button
                  onClick={() =>
                    setCurrentPdfPage((prev) =>
                      Math.min(pdfPages[currentPreviewFile.id] || 1, prev + 1)
                    )
                  }
                  disabled={
                    currentPreviewFile?.file.type !== "application/pdf" ||
                    currentPdfPage >= (pdfPages[currentPreviewFile.id] || 1)
                  }
                >
                  <RightArrowIcon />
                </button>
